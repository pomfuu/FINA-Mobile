...
switch(KondisiKendaraan) {
    case "New":
		...
		switch(Simulasi) {
			...
			case "Angsuran":
				...
				String Angsuran = TotalAngsuran
				
				for (int i = 0; i < Angsuran.length(); i++) {
					char digit = Angsuran.charAt(i)
					int keyCode = Character.getNumericValue(digit) + 7
					driver.pressKeyCode(keyCode)
				}
				break;
		...
		Mobile.hideKeyboard()
		Mobile.takeScreenshot((((baseDir + GlobalVariable.screenshotPathBudget)) + '/' + No + '/' + 'New_Car') + '.png', FailureHandling.STOP_ON_FAILURE)
    ...
    case "Used":
		 ...
		 Mobile.takeScreenshot((((baseDir + GlobalVariable.screenshotPathBudget)) + '/' + No + '/' + 'Used_Car') + '.png', FailureHandling.STOP_ON_FAILURE)
...
// Halaman Tenor
Mobile.callTestCase(findTestCase('Test Cases/Simulasi_Budget/HasilHitungan/AngsuranMuka'), null)
Mobile.callTestCase(findTestCase('Test Cases/Simulasi_Budget/HasilHitungan/AngsuranBelakang'), null)
